...
await p.goto(`${BASE}/l/${code}`, { waitUntil: 'networkidle' });
...
// 4. Creator variant.
await p.goto(`${BASE}/l/${code}?c=${creatorId}`, { waitUntil: 'networkidle' });
await p.waitForTimeout(1500);
...
await p.goto(`${BASE}/campaigns/${campaignId}`, { waitUntil: 'networkidle' });
...
await p.goto(`${BASE}/l/definitelynotreal`, { waitUntil: 'networkidle' });
await p.waitForTimeout(1200);
body = await p.textContent('body');
